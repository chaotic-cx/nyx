...
in
{
  meta.maintainers = with lib.maintainers; [ s0me1newithhand7s ]; # for now, at least
  options.chaotic.conduwuit = {
    enable = lib.mkEnableOption "conduwuit service";
        ...
        options = {
          ...
          global.unix_socket_path = lib.mkOption {
            ...
            description = ''
              ...
              `null` (the default value). The option {option}`chaotic.conduwuit.group` must
              be set to a group your reverse proxy is part of.

              This will automatically add a system user "conduwuit" to your system if
              {option}`chaotic.conduwuit.user` is left at the default, and a "conduwuit"
              group if {option}`chaotic.conduwuit.group` is left at the default.
            '';
          };
    ...
    assertions = [
      {
        ...
        message = ''
          In `chaotic.conduwuit.settings.global`, `unix_socket_path` and `address` cannot be set at the
          same time.
      ...
      }
      {
        ...
        message = "If `chaotic.conduwuit.user` is changed, the configured user must already exist.";
      }
      {
        assertion = cfg.group != defaultGroup -> config ? users.groups.${cfg.group};
        message = "If `chaotic.conduwuit.group` is changed, the configured group must already exist.";
      }
    ];
